...
// #2026_05_08_Time_540_ms_(100.00%)_Space_267.97_MB_(76.92%)
...
public class Solution {
    ...
    static MNode buildMerge(int[] arr, int l, int r) {
    ...
    static int countLE(MNode node, int ql, int qr, int x) {
    ...
    static long sumLE(MNode node, int ql, int qr, int x) {
        if (node == null || ql > node.r || qr < node.l) {
            return 0L;
        }
        if (ql <= node.l && node.r <= qr) {
